class ExternalItem::WebhookProcessor
  ...
  def process
    ...
    when [ "ITEM", "ERROR" ]
      ...
      Rails.logger.warn("Unhandled external webhook type: #{webhook_type}:#{webhook_code}")
    end
  rescue => e
    # To always ensure we return a 200 to external provider (to keep endpoint healthy), silently capture and report all errors
  ...
  private
    ...
    def handle_missing_item
      return if external_item.present?

      # If we cannot find an item in our DB, that means we've reached an invalid data state where
      # the External Item (upstream) still exists (and is being billed), but doesn't exist internally.
      #
      ...
      Sentry.capture_exception(MissingItemError.new("Received external webhook for item no longer in our DB.  Manual action required to resolve.")) do |scope|
        scope.set_tags(external_item_id: item_id)
      end
    end
end
