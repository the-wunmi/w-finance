...
class ExternalAccount::Transactions::ProcessorTest < ActiveSupport::TestCase
  ...
  test "processes added and modified plaid transactions" do
    ...
    @external_account.update!(raw_transactions_payload: {
    ...
    ExternalEntry::Processor.expects(:new)
                         .with(added_transactions.first, external_account: @external_account, category_matcher: category_matcher_mock)
                         .returns(mock_processor)
                         .once

    ExternalEntry::Processor.expects(:new)
                         .with(modified_transactions.first, external_account: @external_account, category_matcher: category_matcher_mock)
                         .returns(mock_processor)
                         .once

    mock_processor.expects(:process).twice

    processor = ExternalAccount::Transactions::Processor.new(@external_account)
    processor.process
  end
